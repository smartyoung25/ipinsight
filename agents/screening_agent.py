    ...
    return {
        ...
        "scrReport": {
            ...
            "gateRouting": {
                ...
                "pqeScoreBreakdown": {
                    "core_nodes":      {"weight": 30, "value": core_nodes,          "contrib": int(core_nodes / max(core_nodes, 8) * 30), "label": f"핵심 노드 {core_nodes}개"},
                    "support_coverage":{"weight": 25, "value": round(support_cov,2), "contrib": int(support_cov * 25),                    "label": f"지지 커버리지 {support_cov:.0%}"},
                    "black_box":       {"weight": 20, "value": round(black_box,2),   "contrib": int((1-black_box)*20),                    "label": f"블랙박스 비율 {black_box:.0%} (낮을수록 좋음)"},
                    "legal_status":    {"weight": 15, "value": round(legal_status,2),"contrib": int(legal_status*15),                     "label": f"법적 안정성 {legal_status:.0%}"},
                    "family_coverage": {"weight": 10, "value": round(family_cov,2),  "contrib": int(min(family_cov,1.0)*10),              "label": f"패밀리 커버리지 {family_cov:.0%}"},
                    "total": score,
                    "grade": "Go" if score >= 65 else "Hold" if score >= 40 else "Kill",
                },
